...
export type PositionalArguments = { index: number; value: string }[];
export type TypeOf<T extends ArgType> = T extends "string"
  ? string
  : T extends "number"
    ? number
    : T extends "boolean"
...
export type ArgumentDescriptor<T extends ArgType = ArgType> =
  | ArgumentDescriptorSingle<T>
  | ArgumentDescriptorMultiple<T>;

export type ArgumentsSpecification = { [index: string]: ArgumentDescriptor };
export type ArgumentValidator<T> = (
  value: T,
...
export type OptionalArgs<T extends ArgumentsSpecification> = {
  [K in keyof T]: T[K]["required"] extends true
    ? never
    : T[K]["default"] extends T[K]["default"] & {}
      ? never
      : K;
}[keyof T];
...
export type ParseArgsResult<T extends ParseArgsConfig> = {
  values: ({
    [K in Exclude<
      keyof T["specification"],
      OptionalArgs<T["specification"]>
    >]: T["specification"][K]["multiple"] extends true
      ? TypeOf<T["specification"][K]["type"]>[]
      : TypeOf<T["specification"][K]["type"]>;
  } & {
    [K in OptionalArgs<T["specification"]>]?: TypeOf<T["specification"][K]["type"]>;
  }) &
    (T["allowUnknown"] extends true ? { [x: string]: any } : {});
