...
export const base64: Base64 = Object.freeze({
  ...
  encode: (data: ArrayBuffer | ArrayBufferView | string, urlSafe: boolean = false): string => {
    ...
    while (offset < byteLength) {
      ...
      result[resultOffset++] = alphabet[(triplet & 0xfc0000) >> 18] ?? 0;
      result[resultOffset++] = alphabet[(triplet & 0x3f000) >> 12] ?? 0;
      result[resultOffset++] = alphabet[(triplet & 0xfc0) >> 6] ?? 0;
      result[resultOffset++] = alphabet[triplet & 0x3f] ?? 0;
    }
...
export const convertToDataUrl: ConverToDataUrl = async (
  ...
  if (!Array.isArray(source)) {
    bytes = ArrayBuffer.isView(source)
    ...
    for (const s of source) {
      if (ArrayBuffer.isView(s)) buffers.push(Buffer.from(s.buffer, s.byteOffset, s.byteLength));
      else if (s instanceof ArrayBuffer) buffers.push(Buffer.from(s));
      else buffers.push(await s.bytes());
    }
  ...
  return mimeType ? `data:${mimeType};base64,${base64String}` : `data;base64,${base64String}`;
};
